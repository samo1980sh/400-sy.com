...
<!-- CARD_HOVER_SECOND_IMAGE_01_START -->
<script data-card-hover-second-image-01>
(function frontProductCardHoverSecondImage() {
    'use strict';

    var hoverMedia = window.matchMedia ? window.matchMedia('(hover: hover) and (pointer: fine)') : null;

    function desktopHoverEnabled() {
        return !hoverMedia || hoverMedia.matches;
    }
    ...
    function imageUrlFromElement(img) {
        ...
        return clean(img.getAttribute('data-src') || img.currentSrc || img.getAttribute('src') || img.src || '');
    }
    ...
    function splitUrl(url) {
        ...
        if (queryIndex >= 0) {
            query = cleanUrl.slice(queryIndex);
    ...
    function hoverCandidatesFromUrl(url) {
        ...
        var ext = match[2];
        var candidates = [];
        var numberedMatch = base.match(/^(.*)-([0-9]+)$/);

        if (numberedMatch) {
            if (numberedMatch[2] !== '1') {
                candidates.push(numberedMatch[1] + '-1' + ext + parts.query + parts.hash);
            }
        } else {
            candidates.push(base + '-1' + ext + parts.query + parts.hash);
        }
    ...
    function preloadFirstAvailable(candidates, done) {
        ...
        function next() {
            ...
            if (preloadCache[url] === true) {
                done(url);
                return;
            }

            if (preloadCache[url] === false) {
                next();
                return;
            }
    ...
    function cardFromTarget(target) {
        return target ? target.closest('.card-product') : null;
    }

    function imageFromCard(card) {
        return card ? card.querySelector('.img-product') : null;
    }

    function activeSwatch(card) {
        if (!card) {
            return null;
        }

        return card.querySelector('.list-color-item.color-swatch.active') || card.querySelector('.list-color-item.color-swatch');
    }

    function swatchImage(swatch) {
        if (!swatch) {
            return '';
        }

        return clean(swatch.getAttribute('data-color-image') || (swatch.dataset ? swatch.dataset.colorImage : '') || '');
    }

    function currentMainImage(card, img) {
        var swatch = activeSwatch(card);
        var fromSwatch = swatchImage(swatch);

        if (fromSwatch) {
            return fromSwatch;
        }

        return clean((img && img.dataset ? img.dataset.cardCurrentImage : '') || imageUrlFromElement(img));
    }

    function rememberCurrentImage(card) {
        var img = imageFromCard(card);

        if (!card || !img) {
            return;
        }

        var main = currentMainImage(card, img);

        if (main && img.dataset) {
            img.dataset.cardCurrentImage = main;
        }
    }

    function enterCard(card) {
        if (!desktopHoverEnabled() || !card || card.dataset.cardHoverSecondActive === '1') {
            return;
        }
        ...
        var mainImage = currentMainImage(card, img);

        if (!mainImage) {
            return;
        }

        card.dataset.cardHoverSecondActive = '1';
        img.dataset.cardHoverReturnImage = mainImage;
        img.dataset.cardCurrentImage = mainImage;

        preloadFirstAvailable(hoverCandidatesFromUrl(mainImage), function (hoverImage) {
            if (card.dataset.cardHoverSecondActive !== '1' || !hoverImage) {
                return;
            }

            setImageUrl(img, hoverImage);
            card.classList.add('card-product-hover-second-image-active');
        });
    }
    ...
    function leaveCard(card) {
        ...
        card.dataset.cardHoverSecondActive = '0';
        card.classList.remove('card-product-hover-second-image-active');

        var restoreImage = currentMainImage(card, img) || clean(img.dataset.cardHoverReturnImage || img.dataset.cardCurrentImage || '');

        if (restoreImage) {
            img.dataset.cardCurrentImage = restoreImage;
            setImageUrl(img, restoreImage);
        }
    }

    document.addEventListener('mouseover', function (event) {
        var wrapper = event.target.closest('.card-product-wrapper.hover-img, .card-product .collection-image');

        if (!wrapper || wrapper.contains(event.relatedTarget)) {
            return;
        }

        enterCard(cardFromTarget(wrapper));
    }, true);
    ...
    document.addEventListener('mouseout', function (event) {
        ...
        leaveCard(cardFromTarget(wrapper));
    }, true);

    document.addEventListener('mouseover', function (event) {
        var swatch = event.target.closest('.card-product .list-color-item.color-swatch');

        if (!swatch) {
            return;
        }

        var card = cardFromTarget(swatch);

        window.setTimeout(function () {
            rememberCurrentImage(card);
        }, 0);
    }, true);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('.card-product').forEach(rememberCurrentImage);
    });
})();
</script>
<!-- CARD_HOVER_SECOND_IMAGE_01_END -->
