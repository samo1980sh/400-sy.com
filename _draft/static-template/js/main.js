...
(function ($) {
  ...
  var countrySelector = function () {
    ...
    $(".js-country-select").on("changed.bs.select change", function () {
  ...
  var currencySelector = function () {
    ...
    var storageKey = "selectedCurrency";
    var savedCurrency = localStorage.getItem(storageKey);
    var allowedCurrencies = ["SYP", "EGP", "JOD"];

    if (savedCurrency && allowedCurrencies.includes(savedCurrency)) {
      $(".js-currency-select").val(savedCurrency);
      if ($.fn.selectpicker) {
        $(".js-currency-select").selectpicker("refresh");
      }
    } else {
      $(".js-currency-select").val("SYP");
      localStorage.setItem(storageKey, "SYP");
      if ($.fn.selectpicker) {
        $(".js-currency-select").selectpicker("refresh");
      }
    }

    $(".js-currency-select").on("changed.bs.select change", function () {
  ...
  var totalPriceVariant = function () {
    ...
    $(".color-btn, .size-btn").on("click", function () {
      ...
      quantityInput.val(1);
      $(".price-on-sale").text("SYP" + newPrice.replace(/\B(?=(\d{3})+(?!\d))/g, ","));
      var totalPrice = newPrice;
      $(".total-price").text("SYP" + totalPrice.replace(/\B(?=(\d{3})+(?!\d))/g, ","));
    });
  ...
  var cartLineTotals = function () {
    ...
    function updateMiniCartItem($item) {
      var unitPrice =
        parseInt($item.attr("data-unit-price"), 10) ||
      ...
      $item.find(".price").first().text(formatMoney(unitPrice * qty));
    }

    function updateMiniCartSubtotal() {
      var subtotal = 0;
      $(".modal-shopping-cart .tf-mini-cart-item").each(function () {
        subtotal += parseMoney($(this).find(".price").first().text());
      });
      $(".modal-shopping-cart .tf-totals-total-value").text(formatMoney(subtotal));
    }
